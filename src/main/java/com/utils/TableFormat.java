...
public class TableFormat {
	...
	public static void displayPlayers(List<PlayerEntity> playerList) {
		List<String[]> playerTable = new ArrayList<>();
		String[] header = createTableRow("PLAYER ID", "PLAYER NAME", "POSITIONS");
		playerTable.add(header);
		for (PlayerEntity player : playerList) {
			String[] row = TableFormat.createTableRow(
					Converter.convertToPlayerIdInteger(player.getPlayerId()).toString(), player.getPlayerName(),
					player.getPlayerPositions());
			playerTable.add(row);
		}
