...
public class DatabaseImport {
	...
	private DatabaseImport() {
		try {
			...
			logService.log(Level.SEVERE,
					"ERROR! Exception occurred while fetching from clubs table" + e.getStackTrace());
		}
		...
		try {
			...
			System.out.println("Database errored out");
			logService.log(Level.SEVERE,
					"ERROR! Exception occurred while fetching from nationality table" + e.getStackTrace());
		}
		setClubs();
		...
		try {
			...
			System.out.println("Database errored out");
			logService.log(Level.SEVERE,
					"ERROR! Exception occurred while storing players table into variable" + e.getStackTrace());
		}
	}
	...
	private static void fetchNationality() throws Exception {
		...
		while (rs.next()) {
			table_nationality.put(rs.getString("nationality_id"), rs.getString("nationality"));
		}
	}
	...
	private static void fetchClubs() throws Exception {
		...
		while (rs.next()) {
			...
			table_clubs.put(rs.getString("club_id"),
					Arrays.asList(rs.getString("club_name"), club_attributes, rs.getLong("transfer_budget")));
		}
	}
	...
	private static void setClubs() {
		...
		for (Map.Entry<String, List<Object>> club : table_clubs.entrySet()) {
			...
			clubs[i] = new ClubModel(Integer.parseInt(club.getKey().replace('C', '0')),
					club.getValue().get(0).toString(), (HashMap<ClubAttributes, Integer>) club.getValue().get(1),
	...
	private static PlayerPositions[] getPositions(String s) {
		...
		for (String position : positions)
			temp_pos.add(PlayerPositions.valueOf(PlayerPositions.class, position));
	...
	private static void setPlayers() throws Exception {
		...
		while (rs.next()) {
			...
			players[i] = new PlayerModel(Integer.parseInt(rs.getString("player_id").replace("P", "0")),
					rs.getString("player_name"), getPositions(rs.getString("player_positions")), rs.getInt("overall"),
					rs.getInt("potential"), rs.getLong("value_eur"), rs.getInt("age"), rs.getInt("height_cm"),
					rs.getInt("weight_kg"), (String) table_clubs.get(rs.getString("club_id")).get(0),
					PlayerPositions.valueOf(PlayerPositions.class, rs.getString("club_position")),
					rs.getInt("club_jersey_number"), rs.getInt("club_contract_valid_until"),
					table_nationality.get(rs.getString("nationality_id")), rs.getString("preferred_foot"),
					rs.getInt("skill_moves"),
					PlayerWorkRate.valueOf(PlayerWorkRate.class,
							Arrays.asList(rs.getString("work_rate").split("/")).get(0).toUpperCase()),
					(rs.getString("player_traits").equals("NA") ? List.of().toArray(empty_array)
							: Arrays.asList(rs.getString("player_traits").split("(, )")).toArray(empty_array)),
					player_attributes, false, 0, true);
			i++;
		}
	}
}
